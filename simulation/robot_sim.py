...
class MobileManipulator:
    ...
    def update_controller(self, dt: float) -> None:
        """Advance targets continuously from the current held-key state."""
        self.targets.yaw = self._clip(
            self.targets.yaw + self.keys.axis(glfw.KEY_1, glfw.KEY_2) * ARM_RATE * dt,
            -150.0 * DEG,
            150.0 * DEG,
        )
        self.targets.shoulder = self._clip(
            self.targets.shoulder + self.keys.axis(glfw.KEY_3, glfw.KEY_4) * ARM_RATE * dt,
            -92.0 * DEG,
            92.0 * DEG,
        )
        self.targets.elbow = self._clip(
            self.targets.elbow + self.keys.axis(glfw.KEY_5, glfw.KEY_6) * ARM_RATE * dt,
            -125.0 * DEG,
            125.0 * DEG,
        )
        self.targets.aperture = self._clip(
            self.targets.aperture + self.keys.axis(glfw.KEY_7, glfw.KEY_8) * GRIPPER_RATE * dt,
            0.0,
            0.045,
        )
        self._apply_targets()
